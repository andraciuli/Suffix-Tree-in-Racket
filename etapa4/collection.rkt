...
(define-syntax-rule (collection-empty? s)
  (stream-empty? s))

; Returnează primul element al colecției
(define-syntax-rule (collection-first s)
  (stream-first s))

; Returnează restul colecției
(define-syntax-rule (collection-rest s)
  (stream-rest s))

(define-syntax-rule (collection-map f s)
  (stream-map f s))

(define-syntax-rule (collection-filter f s)
  (stream-filter f s))